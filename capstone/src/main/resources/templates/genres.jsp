...
<body>
	<div class="container">
		...
		<div class="row">
			<div class="col-10">
					<a class="btn btn-primary btn-lg" href="/admin/addGenre" role="button">Add Genre</a></br>
			</div>
			<div class="col">
		...
		<table id="example" class="table table-bordered table-striped table-light table-hover">
			...
			<tbody>
				<tr th:each="g,state : ${genres}">
		           <td th:text="${state.count}" />
		           <td th:text="${g.name}" />
		           <td>
		           	<a th:href="@{/admin/updateGenre(genreId=${g.id})}" class="btn btn-info btn-sm">Update</a>
		           	<a th:href="@{/admin/deleteGenre(genreId=${g.id})}" class="btn btn-danger btn-sm"
		           	onclick="if (!(confirm('Are you sure you want to delete this genre?'))) return false;">Delete</a>		
           		  </td>
			     </tr>
			</tbody>
		</table>
